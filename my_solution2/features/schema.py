"""
create_features6.py — feature set v4 for Poker44.

Reimplementacja zestawu cech najlepszego minera (top1). Dwupoziomowa konstrukcja:
  1. _hand_features: ~40 cech NA RĘKĘ (action shares, entropie, sizing, pot,
     stacki, hero/button position).
  2. chunk_features: agreguje KAŻDĄ cechę per-ręka przez 7 statystyk
     (mean/std/min/max/q10/q50/q90) po wszystkich rękach chunka => ~280 cech.
  3. Rodzina SIGNATURES: koduje sekwencję akcji/aktorów/streetów/sizingu każdej
     ręki jako krotkę i mierzy ich powtarzalność w chunku (top_share, unique_share)
     -> mocny detektor regularności botów.

Wszystkie cechy mają prefiks 'schema_' (oprócz hand_count), więc nie kolidują z
create_features3/4/5. Zwraca dict, jak pozostałe ekstraktory.

Użycie:
    from create_features6 import extract_chunk_features_v4
    feats = {**extract_chunk_features(g),       # v1
             **extract_chunk_features_v2(g),     # v2
             **extract_chunk_features_v3(g),     # v3
             **extract_chunk_features_v4(g)}     # v4 (ten plik)
"""
...
def _hand_features(hand: dict[str, Any]) -> dict[str, float]:
    ...
    monotonic = sum(1 for prev, cur in zip(pot_after, pot_after[1:]) if cur + 1e-9 >= prev)
    ...
    return {
        ...
        "schema_pot_growth_bb": (max(pot_after) - min(pot_before) if pot_after and pot_before else 0.0),
...
def extract_chunk_features_v4(chunk: list[dict[str, Any]]) -> Optional[dict[str, float]]:
    """Główny ekstraktor v4. Zwraca dict cech (lub None gdy pusty chunk,
    spójnie z create_features3/4/5)."""
    if not chunk:
        return None
    ...
    # agregacja każdej cechy per-ręka przez 7 statystyk
    for name in feature_names:
        series = [float(features[name]) for features in per_hand]
        _aggregate_feature(name, series, out)

    # rodzina SIGNATURES — powtarzalność sekwencji w chunku
    ...
    for hand, feats in zip(chunk, per_hand):
        ...
        actor_seq = tuple(
            _safe_int((a or {}).get("actor_seat"), 0) for a in actions
            if _safe_int((a or {}).get("actor_seat"), 0) > 0)
        street_seq = tuple(str((a or {}).get("street") or "").lower().strip() for a in actions)
        amounts = [max(0.0, _safe_float((a or {}).get("normalized_amount_bb"), 0.0)) for a in actions]
